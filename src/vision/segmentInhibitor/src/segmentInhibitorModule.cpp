...
#include "iCub/segmentInhibitorModule.h"

bool segmentInhibitorModule::configure(ResourceFinder &rf) {
    ...
    pThread = new segmentInhibitorThread(moduleName);
...
bool segmentInhibitorModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}


bool segmentInhibitorModule::respond(const Bottle& command, Bottle& reply)
{
...
bool segmentInhibitorModule::updateModule()
{
    return true;
}

double segmentInhibitorModule::getPeriod()
{
    /* module periodicity (seconds), called implicitly by myModule */
    return 1;
}

bool segmentInhibitorModule::close() {
