...
selectiveAttentionProcessor::selectiveAttentionProcessor(int rateThread):PeriodicThread(rateThread) {
    ...
    cart3_yarp       = new ImageOf <PixelMono>; // 3rd cartesian image
...
void selectiveAttentionProcessor::reinitialise(int width, int height){
    ...
    cart3_yarp             = new ImageOf<PixelMono>;
    ...
    cart3_yarp->resize(xSizeValue,ySizeValue);
...
bool selectiveAttentionProcessor::threadInit(){
    ...
    cart3Port.open(             getName("/cart3:i").c_str());
...
void selectiveAttentionProcessor::run(){
    ...
    if(!interrupted){
        ...
        if((outputCmdPort.getOutputCount())&&(handFixation)) {
            ...
            commandBottle.addFloat32(x(0));
            commandBottle.addFloat32(x(1));
            commandBottle.addFloat32(x(2));
            outputCmdPort.write();
            return; 
        }                        
        ...
        //kc2 = 0;
        double sumK = k1 + k2 + k3 + k4 + k5 + k6 + kmotion + kc1 + kc2 + kc3;
        ...
        if((cart3Port.getInputCount())&&(kc3!=0)) {
            tmp = cart3Port.read(false);
            if(tmp!= 0) {
                copy_8u_C1R(tmp,cart3_yarp);
                //idle=false;
            }
        }
...
cartSpace:
        ...
        if((inhiCartPort.getInputCount())&&(portionRequestPort.getOutputCount())) {
            ...
            sent->addFloat32(angles[0]);
            sent->addFloat32(angles[1]);
        ...
        */
        ...
        unsigned char* pcart3    = cart3_yarp->getRawImage();
        ...
        for(int y=0; (y < ySizeValue) && (!maxResponse); y++) {
            for(int x=0; (x < xSizeValue) && (!maxResponse); x++) {
                double combinValue = (double) (*pcart1 * (kc1/sumK) + *pcart2 * (kc2/sumK) + *pcart3 * (kc3/sumK) + *pInter * ((k1 + k2 + k3 + k4 + k5 + k6)/sumK) + *pmotion * (kmotion/sumK));
				...
				***************************************************/
                ...
                if(outputCmdPort.getOutputCount()){
                    if(!handFixation) {            
                        ...
                        commandBottle.addFloat32(z);
                        commandBottle.addFloat32(timing);
                        outputCmdPort.write();
                    } 
                }
...
bool selectiveAttentionProcessor::outPorts(){
    ...
    if(feedbackPort.getOutputCount()){
        ...
        if(dif > 30 + 2){
            ...
            commandBottle.clear();
            commandBottle.addVocab32(yarp::os::createVocab32('s','e','t'));
            commandBottle.addVocab32(yarp::os::createVocab32('k','t','d'));
            salienceTD=salienceTD+0.1;
        
            //if(salienceTD>0.99)
                salienceTD=1.0;
            printf("salienceTD \n");
            commandBottle.addFloat32((double)salienceTD);
            feedbackPort.write(commandBottle,in);
            commandBottle.clear();
            commandBottle.addVocab32(yarp::os::createVocab32('s','e','t'));
            commandBottle.addVocab32(yarp::os::createVocab32('k','b','u'));
            salienceBU=salienceBU-0.1;
            
            //if(salienceBU<=0)
                salienceBU=0;
            commandBottle.addFloat32((double)salienceBU);
            feedbackPort.write(commandBottle,in);    
            ...
            commandBottle.clear();
            commandBottle.addVocab32(yarp::os::createVocab32('s','e','t'));
            commandBottle.addVocab32(yarp::os::createVocab32('r','i','n'));
            commandBottle.addFloat32((double)targetRed);
            //commandBottle.addFloat32(255.0);
            feedbackPort.write(commandBottle,in);
            commandBottle.clear();
            commandBottle.addVocab32(yarp::os::createVocab32('s','e','t'));
            commandBottle.addVocab32(yarp::os::createVocab32('g','i','n'));
            commandBottle.addFloat32((double)targetGreen);
            //commandBottle.addFloat32(0.0);
            feedbackPort.write(commandBottle,in);
            commandBottle.clear();
            commandBottle.addVocab32(yarp::os::createVocab32('s','e','t'));
            commandBottle.addVocab32(yarp::os::createVocab32('b','i','n'));
            commandBottle.addFloat32((double)targetBlue);
            feedbackPort.write(commandBottle,in);
            commandBottle.clear();
            //commandBottle.addFloat32(0.0);
            printf("%f,%f,%f \n",(double)targetRed,(double)targetGreen,(double)targetBlue);
...
void selectiveAttentionProcessor::magnoCellularSuppression(bool on) {
    if(on) {
        ...
        commandBottle.clear();
        commandBottle.addVocab32(yarp::os::createVocab32('s','u','s'));
        ...
        Bottle& commandBottle = magnoCellFeedback.prepare();
        commandBottle.clear();
        commandBottle.addVocab32(yarp::os::createVocab32('r','e','s'));
        magnoCellFeedback.write();
    }
}
