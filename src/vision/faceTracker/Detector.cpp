...
bool Detector::threadInit() {
    
}

void Detector::suspend() {
    idle = true;
    yInfo("Suspending the thread");
    Time::delay(0.5);
    iGaze->stopControl();
    //clientGaze->close();
}
...
void Detector::detectAndDraw(cv::Mat& img, double scale, circle_t &c)
{
    ...
    if(rectTopLeft.x != 0 && rectTopLeft.y != 0 && rectBottomRight.x != 0 && rectBottomRight.y != 0){
        ...
        faceRectCoordinateBottle.addVocab32(createVocab32('s','e','t'));
        faceRectCoordinateBottle.addVocab32(createVocab32('t','r','a','c'));
