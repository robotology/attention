...
#include "iCub/gaussianInhibitorModule.h"


using namespace attention::dictionary;

bool gaussianInhibitorModule::configure(ResourceFinder &rf) {
    ...
    pThread = new gaussianInhibitorThread(moduleName);
...
bool gaussianInhibitorModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}


bool gaussianInhibitorModule::respond(const Bottle& command, Bottle& reply)
{
    ...
    if (command.get(0).asString()=="quit") {
        ...
        pThread->removeAllPoints();
        reply.addString("ok");
        return true;
    }else if (command.get(0).asVocab32() == COMMAND_VOCAB_SET) {
        if(command.size()==3){
            if(pThread->setValue(command.get(1),command.get(2))){
                reply.addString("ok");
                return true;
            }
        }
    }else if (command.get(0).asVocab32() == COMMAND_VOCAB_GET) {
        if(command.size()==2){
            double returnValue = pThread->getValue(command.get(1));
            reply.addFloat64(returnValue);
            return true;
        }

    }
    reply.addString("Error in the command");

    return true;
}

/* Called periodically every getPeriod() seconds */
bool gaussianInhibitorModule::updateModule()
{
    return true;
}

double gaussianInhibitorModule::getPeriod()
{
    /* module periodicity (seconds), called implicitly by myModule */
    return 1;
}

bool gaussianInhibitorModule::close() {
