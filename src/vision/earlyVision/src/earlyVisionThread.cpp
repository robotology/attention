...
earlyVisionThread::earlyVisionThread(): PeriodicThread(1.0/RATE_OF_INTEN_THREAD)
{
...
void earlyVisionThread::resize(int width_orig, int height_orig)
{
    ...
    cs_tot_32f = new cv::Mat(cv::Size(width_orig, height_orig),CV_32FC1);
    colcs_out = new cv::Mat(cv::Size(width_orig, height_orig),CV_8UC1);
    ycs_out = new cv::Mat(cv::Size(width_orig, height_orig),CV_8UC1);
    scs_out = new cv::Mat(cv::Size(width_orig, height_orig),CV_8UC1);
    vcs_out = new cv::Mat(cv::Size(width_orig, height_orig),CV_8UC1);
...
void earlyVisionThread::extractPlanes()
{
    ...
    for (int r = 0; r < h; r++)
    {
        for (int c = 0; c < w; c++)
        {
            ...
            *ptrIntensityImg = ONE_BY_ROOT_THREE * sqrt(
                (double)(*shift[0] * *shift[0] + *shift[1] * *shift[1] + *shift[2] * *shift[2]));
...
void earlyVisionThread::centerSurrounding()
{
    ...
    if (true){
        ...
        cv::Mat unXtnYplaneMat = yarp::cv::toCvMat(*unXtnYplane);
        cv::Mat _YMat = yarp::cv::toCvMat(_Y);
        cv::Mat YofYUVpyMat = yarp::cv::toCvMat(*YofYUVpy);

        centerSurr->proc_im_8u(&unXtnYplaneMat,&_YMat);
        centerSurr->get_pyramid_gauss(0)->convertTo(YofYUVpyMat,CV_8UC1,255,0);

        // Send it to other thread
        cv::Mat *toSend[GABOR_SCALES], *toSendGauss[GABOR_SCALES];
        ...
        if (isYUV)
        {
            ...
            cv::Mat unXtnUplaneMat = yarp::cv::toCvMat(*unXtnUplane);
            centerSurr->proc_im_8u(&unXtnUplaneMat, scs_out);
            cv::add(*centerSurr->get_centsur_32f(), *cs_tot_32f, *cs_tot_32f); // in place?

            cv::Mat UofYUVpyMat = yarp::cv::toCvMat(*UofYUVpy);
            centerSurr->get_pyramid_gauss(0)->convertTo(UofYUVpyMat,CV_8UC1,255,0);

            cv::Mat unXtnVplaneMat = yarp::cv::toCvMat(*unXtnVplane);
            //Colour process V:performs centre-surround uniqueness analysis:
            centerSurr->proc_im_8u(&unXtnVplaneMat, vcs_out);
            cv::add(*centerSurr->get_centsur_32f(), *cs_tot_32f, *cs_tot_32f);

            cv::Mat VofYUVpyMat = yarp::cv::toCvMat(*VofYUVpy);
            centerSurr->get_pyramid_gauss(0)->convertTo(VofYUVpyMat,CV_8UC1,255,0);

            //get min max
            double valueMin = 1000;
            double valueMax = -1000;
            //img_UV->zero();     // this is not strictly required
            cv::minMaxLoc(*cs_tot_32f, &valueMin, &valueMax);
            ...
            cv::Mat _UVMat = yarp::cv::toCvMat(_UV);
            cs_tot_32f->convertTo(_UVMat,CV_8UC1,255 / (valueMax - valueMin),-255 * valueMin / (valueMax - valueMin));
            ...
            for (int i = 0; i < htYUV; ++i)
            {
                for (int j = 0; j < wdYUV; ++j)
                {
                    ...
                    green = *ptrYplane - (0.698001 * ((double)*ptrVplane - 128.0)) - (0.337633 * ((double)*ptrUplane -
                        128.0));
            ...
            cv::Mat unXtnVplaneMat = yarp::cv::toCvMat(*unXtnVplane);
            cv::Mat _UVMat = yarp::cv::toCvMat(_UV);
            cv::Mat _VMat = yarp::cv::toCvMat(_V);
            centerSurr->proc_im_8u(&unXtnUplaneMat, &_UVMat);

            //Colour process V:performs centre-surround uniqueness analysis:
            centerSurr->proc_im_8u(&unXtnVplaneMat, &_VMat);
        }
...
void earlyVisionThread::addFloatImage(cv::Mat* sourceImage, cv::Mat* toBeAddedImage, double multFactor, double shiftFactor)
{

    assert(sourceImage->size() == toBeAddedImage->size() && sourceImage->type() == toBeAddedImage->type());
    *sourceImage += (*toBeAddedImage * multFactor + shiftFactor);
