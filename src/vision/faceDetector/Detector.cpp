               ...
               // we found an stable face
                // if(++counter > certainty)
                {
                    if(disableGazeControl){
                        cvCircle(display, cvPoint(cvRound(face.x), cvRound(face.y)), 2, cvScalar(0, 255, 0,0), 3, 8, 0 );
                        cvCircle(display, cvPoint(cvRound(face.x), cvRound(face.y)), cvRound(face.r), cvScalar(0,255,0,0), 2, 8, 0 );
