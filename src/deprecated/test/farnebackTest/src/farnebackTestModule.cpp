...
/**
 * @file farnebackTestModule.cpp
 * @brief Implementation of the farnebackTestModule (see header file).
 */

#include "iCub/farnebackTestModule.h"
...
bool farnebackTestModule::configure(yarp::os::ResourceFinder &rf) {
	...
	*/ 
    ...
    rThread = new farnebackTestThread(robotName, configFile);
...
bool farnebackTestModule::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool farnebackTestModule::close() {
    handlerPort.close();
    /* stop the thread */
...
bool farnebackTestModule::respond(const Bottle& command, Bottle& reply) 
{
    bool ok = false;
...
bool farnebackTestModule::updateModule()
{
    return true;
}

double farnebackTestModule::getPeriod()
{
