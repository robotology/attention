...
/**
 * @file batchAnalysis2Module.cpp
 * @brief Implementation of the batchAnalysis2Module (see header file).
 */

#include "iCub/batchAnalysis2Module.h"
...
bool batchAnalysis2Module::configure(yarp::os::ResourceFinder &rf) {
    /* Process all parameters from both command-line and .ini file */

    /* get the module name which will form the stem of all module port names */
    moduleName            = rf.check("name", 
                           Value("/batchAnalysis2"), 
	...
	*/ 
	double th1Level = rf.check("threshold1Level", 
                           Value(0.5), 
                           "threshold 1 level for the segmentation (float)").asFloat32();
	...
	*/ 
	double th2Level = rf.check("threshold2Level", 
                           Value(0.5), 
                           "threshold 2 level for the segmentation (float)").asFloat32();
	...
	*/ 
	double th3Level = rf.check("threshold3Level", 
                           Value(0.5), 
                           "threshold 3 level for the segmentation (float)").asFloat32();
    ...
    rThread = new batchAnalysis2Thread(robotName, configFile);
...
bool batchAnalysis2Module::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool batchAnalysis2Module::close() {
    handlerPort.close();
    /* stop the thread */
...
bool batchAnalysis2Module::respond(const Bottle& command, Bottle& reply) 
{
    bool ok = false;
            ...
            case COMMAND_VOCAB_ALGO:
                {
                    ...
                    case COMMAND_VOCAB_FB:
                        ...
                        reply.clear();
                        reply.addVocab32(COMMAND_VOCAB_FB);
					...
					case COMMAND_VOCAB_TV:
                        ...
                        reply.clear();
                        reply.addVocab32(COMMAND_VOCAB_TV);
					...
					case COMMAND_VOCAB_LK:
                        ...
                        reply.clear();
                        reply.addVocab32(COMMAND_VOCAB_LK);
...
bool batchAnalysis2Module::updateModule()
{
    return true;
}

double batchAnalysis2Module::getPeriod()
{
