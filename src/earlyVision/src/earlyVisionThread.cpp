...
visualFilterThread::visualFilterThread() {
    ...
    logGaborImg = new ImageOf<PixelRgb>;
    logGaborImg->resize(320,240);
    logGaborOutImg = new ImageOf<PixelFloat>;
    logGaborOutImg->resize(320,240);
...
bool visualFilterThread::threadInit() {
    ...
    if (!logGabPortIn.open(getName("/LogGabor:i").c_str())) {
        cout <<": unable to open port "  << endl;
        return false;  // unable to open; let RFModule know so that it won't run
    }
    if (!logGabPortOut.open(getName("/LogGabor:o").c_str())) {
        cout <<": unable to open port "  << endl;
        return false;  // unable to open; let RFModule know so that it won't run
    }
...
void visualFilterThread::orientation() {
    ...
    myRes1->resize(320,240);
    myRes2->resize(320,240);
    
    ImageOf<PixelMono>* myMono;
    myMono = new ImageOf<PixelMono>;
    myMono->resize(320,240);
    ...
    while(true){
    logGaborImg = logGabPortIn.read(true); // intensity image
    ...
    //cvCvtColor((IplImage*)logGaborImg->getIplImage(),im,CV_RGB2GRAY);
    //cvWaitKey(2);

   //ImageOf<PixelRgb>* myImg;
    //myImg = new ImageOf<PixelRgb>;
    //myImg->resize(im->width,im->height);
    //myImg->wrapIplImage(imRGB);
    
    myMono->resize(logGaborImg->width(),logGaborImg->height());
    uchar* oriRGB = (uchar*) logGaborImg->getRawImage();
    uchar* oriMono = (uchar*) myMono->getRawImage();
    uchar* ptrRgb;
    uchar* ptrMono;
    for(int i=0; i<logGaborImg->height();++i){
        ptrRgb = oriRGB + i*logGaborImg->getRowSize();
        ptrMono = oriMono + i*myMono->getRowSize();
        for(int j=0; j<logGaborImg->width();++j){
...
*/
    ...
    if((myRes1 != 0) && (logGabPortOut.getOutputCount())) {
                logGabPortOut.prepare() = *(myRes1);
                logGabPortOut.write();
            }
...
void visualFilterThread::onStop() {
    ...
    logGabPortIn.interrupt();
    logGabPortOut.interrupt();
