...
void featExtractorThread::run() {
    ...
    if(outputPortDescr.getOutputCount() || outputPortPlot.getOutputCount()) {
        ...
        if(tempReady){
            ...
            ACC.at<float>(0,0) =  VEL.at<float>(0,0) - cv::mean(Ut_1, Maskt_18U).val[0]; 
            ACC.at<float>(0,1) = VEL.at<float>(0,1) - cv::mean(Vt_1, Maskt_18U).val[0]; 
            ACC.at<float>(0,2) = 0.0;
            ...
            if (    (V!=VEL.at<float>(0,2))  && C!=0.0 ){ 
                int filtering_signal=1;
                if (filtering_signal==1){
                    ...
                    //currentNormalizedSmoothedV=(currentSmoothedV-Vmin)/(Vmax-Vmin+0.000001);      //+0.000001 because at the beginning Vmax=Vmin=currentSmoothedV
                    //currentNormalizedSmoothedC=(currentSmoothedC-Cmin)/(Cmax-Cmin+0.000001);
                    //currentNormalizedSmoothedR=(currentSmoothedR-Rmin)/(Rmax-Rmin+0.000001);
                    //currentNormalizedSmoothedA=(currentSmoothedA-Amin)/(Amax-Amin+0.000001);
                }
                ...
                if(outputPortDescr.getOutputCount()) {
                    if(contentBottle.get(0).asDouble() !=0.0 ){
