...
/**
 * @file farnebackTestThread.cpp
 * @brief Implementation of the eventDriven thread (see farnebackTestThread.h).
 */
...
farnebackTestThread::farnebackTestThread() {
    robot = "icub";        
}

farnebackTestThread::farnebackTestThread(string _robot, string _configFile){
    robot = _robot;
...
bool farnebackTestThread::threadInit() {
...
void farnebackTestThread::setName(string str) {
    this->name=str;
}


std::string farnebackTestThread::getName(const char* p) {
    string str(name);
    str.append(p);
    return str;
}

void farnebackTestThread::setInputPortName(string InpPort) {
    
}

void farnebackTestThread::visualizationResume(){
    pt->resume();
}

void farnebackTestThread::visualizationSuspend(){
    pt->suspend();
}

bool farnebackTestThread::test(){
    bool reply = true;
    ...
    while (isStopping() != true) {
        ...
        if(!idle){
            if(inputPort.getInputCount()) {
                ...
                ////inputImage=00000000.ppm;


               // generates the outputImage which is what we want to plot                                
                //    //bbb
...
void farnebackTestThread::acquisitionSequence(){
    name = seq +"_"+ alg;
    foldername = "C:/Users/AVignolo/Desktop/Acquisizioni_iCub/"+seq+"/Images/";
    numbername1 [50];
    numbername2 [50];
    number1= sprintf(numbername1, "%8.8d.ppm", k);
    number2= sprintf(numbername2, "%8.8d.ppm", k+1);
    filename1 = foldername+numbername1;
    filename2 = foldername+numbername2;
    image1 = imread(filename1, CV_LOAD_IMAGE_COLOR);   // Read the file
    image2 = imread(filename2, CV_LOAD_IMAGE_COLOR);   // Read the file
    result = processing();
}


void farnebackTestThread::motionToColor(cv::Mat U, cv::Mat V, cv::Mat& colorcodeMatrix){
...
void farnebackTestThread::fakethresholding(cv::Mat& U, cv::Mat& V){
    printf("faketh start \n");
...
void farnebackTestThread::thresholding(cv::Mat& Ut, cv::Mat& Vt, cv::Mat& maskThresholding){
...
void farnebackTestThread::thresholding(cv::Mat Ut_1, cv::Mat Vt_1, cv::Mat& Ut, cv::Mat& Vt, cv::Mat& maskThresholding, std::vector<float>& descr, int& computed){ 
    cv::Mat MAGt = cv::Mat::zeros(Ut.rows, Ut.cols, CV_32FC1);
    cv::Mat THETAt = cv::Mat::zeros(Ut.rows, Ut.cols, CV_32FC1);
...
//void farnebackTestThread::computeFeatures(cv::Mat Ut_1, cv::Mat Vt_1, cv::Mat Ut, cv::Mat Vt, std::vector<float>& descr, int& computed){   //I eliminated CUM
    descr.clear();
...
bool farnebackTestThread::processing(){
    //int dep= image1.depth();
    cv::cvtColor(image1, image1gray, CV_RGB2GRAY);
    cv::cvtColor(image2, image2gray, CV_RGB2GRAY);
    cv::Mat flow;
    cv::calcOpticalFlowFarneback(image1gray, image2gray, flow, 0.2, 3, 19, 10, 7, 1.5, cv::OPTFLOW_FARNEBACK_GAUSSIAN);
    ...
    cv::Mat MV[2]={U,V};
    cv::split(flow, MV);
    cv::Mat colorcodeMatrix =  cv::Mat::zeros(image1.rows,image1.cols, CV_8UC3);
    motionToColor(U, V, colorcodeMatrix);
    //namedWindow( "Display window", WINDOW_AUTOSIZE );// Create a window for display.
    //imshow( "Display window", colorcodeMatrix);                   // Show our image inside it.
...
void farnebackTestThread::threadRelease() {
    // nothing    
}

void farnebackTestThread::onStop() {
    delete inputImage;
	delete outputImage;
